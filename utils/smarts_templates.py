...
logger.setLevel(logger, val=RDLogger.CRITICAL)
...
def get_changed_atoms(reactants, products):
    ...
    if len(prod_atoms) != len(reac_atoms):
        if v:
            print('warning: total number of tagged atoms differ, stechiometry != 1?')
    # err = 1
...
def main(file, writing_path, N=15):
    ...
    try:  # to allow breaking
        for i, example_doc in enumerate(file.readlines()):
            ...
            try:
                # Unpack
                reaction_smiles = example_doc.split(" ")[0].rstrip().replace('"', '')
                all_rxn_strings += reaction_smiles + ","
                ...
                if v:
                    ...
                    print("reactants", [Chem.MolToSmiles(r) for r in reactants])
                    print("agents", [Chem.MolToSmiles(r) for r in agents])
                    print("products", [Chem.MolToSmiles(r) for r in products])
                ...
                # print(i, example_doc)
                all_rxn_strings += "NULL" + ","
                all_rxn_strings += "Could not load SMILES or sanitize"
                continue
            ...
            try:
                ...
                for product in products:
                    if sum([a.HasProp('molAtomMapNumber') for a in product.GetAtoms()]) < len(product.GetAtoms()):
                        ...
                        # print(f'REACTION {i}: {example_doc}')
                        skip = True
                if skip:
                    total_partialmapped += 1
                    all_rxn_strings += "NULL" + ","
                    all_rxn_strings += "ot all product atoms have atom mapping"
                    continue
                ...
                if err:
                    ...
                    all_rxn_strings += "NULL" + ","
                    all_rxn_strings += "Could not get changed atoms"
                    # print(example_doc)
                    continue
                if not changed_atom_tags:
                    # print('No atoms changed?')
                    all_rxn_strings += "NULL" + ","
                    all_rxn_strings += "No atoms changed"
                    # print(example_doc)
                    continue
                ...
                product_fragments = get_fragments_for_changed_atoms(products, changed_atom_tags,
                                                                    radius=0, expansion=expand_changed_atom_tags(
                ...
                if rxn.Validate()[1] != 0:
                    ...
                    # print(example_doc)
                    # print(f'rxn_string {i}: {rxn_string}')
                    all_rxn_strings += "NULL" + ","
                    all_rxn_strings += "Could not validate reaction successfully"
                    continue
                    ...
                    for combination in combinations:
                        ...
                        for j, outcome in enumerate(outcomes):
                            ...
                            for k, product in enumerate(outcome):
                                ...
                                if v:
                                    print(
                                        '  - product {}: {}'.format(k, Chem.MolToSmiles(product, isomericSmiles=True)))
                                    try:
                                        pass
                                    ...
                                    # Draw.MolToFile(product, '{}/rxn_{}/outcome_{}_product_{}.png'.format(folder, i, j, k), size=(250,250))
                                        ...
                                        print('warning: could not draw {}: {}'.format(
                                            Chem.MolToSmiles(product, isomericSmiles=True), e))
                ...
                if correct or DO_NOT_TEST:
                    ...
                    all_rxn_strings += rxn_canonical + ","
                    all_rxn_strings += "success"
                    ...
                    all_rxn_strings += "NULL" + ","
                    all_rxn_strings += "Did not find true product"
                    pass
                total_attempted += 1
    ...
    print('Partially-mapped reactions in {}/{} ({}%) cases'.format(total_partialmapped,
                                                                   total_examples,
                                                                   total_partialmapped * 100.0 / total_examples))
    print('Non-reactions (stereochem only?) in {}/{} ({}%) cases'.format(total_nonreaction,
                                                                         total_examples,
                                                                         total_nonreaction * 100.0 / total_examples))
    ...
    print('Specific product predictions in {}/{} ({}%) cases'.format(total_precise,
                                                                     total_examples,
                                                                     total_precise * 100.0 / total_examples))
